...
export async function initDatabase() {
  ...
  try {
    ...
    try { await connection.query(`ALTER TABLE products ADD COLUMN name_en VARCHAR(255)`); } catch (e) {}
    try { await connection.query(`ALTER TABLE products ADD COLUMN description_en TEXT`); } catch (e) {}
    ...
    if ((adminExists as any[]).length === 0) {
      ...
      await connection.query(
        ...
        ['Admin', 'admin@quickshop.com', hash, 'admin']
      );
    }
